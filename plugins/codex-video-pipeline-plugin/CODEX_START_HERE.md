...
9. Stage 05 当前已锁定主线：`Stage05-A` 只允许在三套本地 Zimage workflow 中选主角色参考图，`Stage05-B` 固定走本地 Qwen NextScene reference-guided workflow；不允许再恢复旧桥接路线。
10. 如果本机缺模型、缺节点、缺工作流导出，必须先明确写出 blocker，不能假装“已完成改造”。
...
13. Stage 05 主线推进必须同时遵守 `docs/STAGE05_MAINLINE_GUARDRAILS.md` 和 `docs/STAGE05_REFERENCE_GUIDED_REFACTOR_PLAN_20260604.md`；未经用户明确批准，不允许从当前主线扩展到其他风格路线、其他镜头集合或其他横向 A/B 试验。
