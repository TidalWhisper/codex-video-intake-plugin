...
def build_repair_packet(
    ...
    return {
        ...
        "project_id": str(character_bible.get("project_id") or storyboard.get("project_id") or script.get("project_id") or brief.get("project_id") or ""),
        ...
        "forbidden_edits": [
            "Do not rewrite the approved story into a different character setup.",
            "Do not change the protagonist identity, role logic, duration context, or locked brief constraints.",
            "Do not remove already-correct reference image planning or readiness fields unless validation requires it.",
        ],
        "current_draft": character_bible,
    }
...
def main(argv: list[str]) -> int:
    if len(argv) != 7:
        print(
            "Usage: python build_stage03_repair_packet.py <locked_brief.json> <script.json> <storyboard.json> <character_bible.json> <validation_errors.json> <output.json>",
            file=sys.stderr,
        )
        return 2
    ...
    packet = build_repair_packet(brief, script, storyboard, character_bible, brief_path, script_path, storyboard_path, character_path, failed_checks)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(packet, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"STAGE03_REPAIR_PACKET_CREATED: {output_path}")
    return 0
