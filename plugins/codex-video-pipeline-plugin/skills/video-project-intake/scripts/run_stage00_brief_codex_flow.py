...
def generate_stage00_brief_llm_output(
    *,
    request_text: str,
    schema_path: Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    refs = references_dir()
    schema_path = refs / "stage00_brief_llm_output.schema.json"
    generation_prompt_path = refs / "stage00_brief_generation_prompt.md"
    repair_prompt_path = refs / "stage00_brief_repair_prompt.md"
    ...
    generate_stage00_brief_llm_output(
    ...
    for attempt_index in range(total_attempts + 1):
        ...
        if not repair_packet_path.exists():
            print(
                "ERROR: Stage 00-B validation failed but no repair packet was created for automatic retry.",
                file=sys.stderr,
            )
            return exit_code

        repair_request_path = intake_dir / f"stage00_brief_codex_repair_request_attempt_{attempt_index + 1}.txt"
        repair_last_message_path = intake_dir / f"stage00_brief_codex_repair_last_message_attempt_{attempt_index + 1}.txt"
