...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 1:
        print("Usage: python confirm_stage02_and_continue.py <project_dir>", file=sys.stderr)
        return 2

    project_dir = Path(args[0]).resolve()
    manifest_path = project_dir / "project_manifest.json"
    storyboard_path = project_dir / "02_storyboard" / "storyboard.json"

    if not manifest_path.exists():
        print(f"ERROR: project manifest not found: {manifest_path}", file=sys.stderr)
        return 1
    if not storyboard_path.exists():
        print(f"ERROR: Stage 02 storyboard not found: {storyboard_path}", file=sys.stderr)
        return 1

    storyboard_data = _load(storyboard_path)
    ok, errors, _warnings = validate_storyboard.validate(storyboard_data, mode="final")
    if not ok:
        print("ERROR: Stage 02 storyboard failed final validation before confirmation:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1

    storyboard_data["status"] = "confirmed"
    storyboard_data["allowed_next_stage"] = "STAGE_03_CHARACTER_BIBLE"
    write_json_file(storyboard_path, storyboard_data)

    manifest_data = _load(manifest_path)
    manifest_data["current_stage"] = "STAGE_02_STORYBOARD_CONFIRMED"
    manifest_data["brief_locked"] = True
    manifest_data["script_confirmed"] = True
    manifest_data["storyboard_confirmed"] = True
    manifest_data["allowed_next_stage"] = "STAGE_03_CHARACTER_BIBLE"
    write_json_file(manifest_path, manifest_data)

    print(f"STAGE02_CONFIRMED: {project_dir}")
