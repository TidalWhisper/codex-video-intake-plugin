...
DEFAULT_NEGATIVE_ANCHOR = (
    "flat illustration, anime cel shading, heavy comic inking, painterly oil canvas, 3D render, "
    "plastic skin, distorted lens anatomy, broken hands, duplicated people, fake product text, logo, watermark"
)
...
def clean_anchor_text(raw: str) -> str:
    ...
    kept: list[str] = []
    for line in lines:
        if line.startswith("YOUR PHOTO:") or line.startswith("YOUR PHOTOGRAPH:"):
            break
...
def choose_photo_anchor(nodes: list[dict[str, Any]]) -> str:
    candidates: list[str] = []
    for node in nodes:
    ...
    preferred_keywords = (
        ...
        "luxury productions",
        "Kodak Portra",
        "documentary photographer chronicling urban life",
    )
    ...
    if candidates:
        return candidates[0]
    return (
        "You are a Hollywood filmmaker making a high-budget film. Your photographs exhibit "
        "atmospheric composition, selective focus, premium studio-grade lighting, and warm-cool cinematic color contrast."
    )
    ...
    *,
    ...
    return {
        "1": {
            ...
            "_meta": {"title": "Load Z-Image Photo UNet"},
        },
        "2": {
            "inputs": {
                "clip_name": clip_name,
            ...
            "_meta": {"title": "Load Z-Image Photo CLIP"},
        },
        "3": {
            "inputs": {
                "vae_name": vae_name,
            ...
            "_meta": {"title": "Load Z-Image Photo VAE"},
        },
        "10": {
            "inputs": {
                "text": "realistic cinematic keyframe",
                "clip": ["2", 0],
            },
        ...
        "11": {
            ...
            "class_type": "CLIPTextEncode",
            "_meta": {"title": "Z-Image Photo Anchor"},
        },
        "12": {
            "inputs": {
                "conditioning_1": ["10", 0],
        ...
        "14": {
            ...
            "class_type": "CLIPTextEncode",
            "_meta": {"title": "Z-Image Photo Negative Anchor"},
        },
        "15": {
            "inputs": {
                "conditioning_1": ["13", 0],
        ...
        "30": {
            ...
            "class_type": "KSampler",
            "_meta": {"title": "Z-Image Photo Bridge Sampler"},
        },
        "40": {
            "inputs": {
                "samples": ["30", 0],
        ...
        "60": {
            "inputs": {
                "images": ["40", 0],
                "filename_prefix": "video/keyframes/zimage_photo_bridge",
            },
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build a Stage 05 API bridge workflow from Amazing Z-Image photo UI graph."
    )
    parser.add_argument("source_ui_graph", help="Path to amazing-z-photo_SAFETENSORS.json UI graph")
    ...
    photo_anchor = choose_photo_anchor(nodes)
    ...
    print(f"STAGE05 ZIMAGE PHOTO BRIDGE WRITTEN: {output_path}")
    return 0
